...
import numpy as np
import torch
from mmcv.image import imresize, impad
...
@TRANSFORMS.register_module()
class SingleImageAdaptiveResize(BaseTransform):
    """Adaptively resize a single image for inference with padding to fit divisor.

    This transform resizes an image while maintaining aspect ratio and pads it
    so that both dimensions are divisible by `size_divisor`. Designed for single-frame inference.

    Args:
        img_size (int): Longest dimension after resizing. Defaults to 640.
        size_divisor (int): Ensures final size is divisible by this value. Defaults to 32.
        extra_pad_ratio (float): Extra padding ratio added before rounding up. Defaults to 0.5.
        keep_aspect_ratio (bool): Whether to maintain original aspect ratio. Defaults to True.
    """
    ...
    def transform(self, results: dict) -> dict:
        """Apply resizing policy to a single image."""
        height, width = results['img_shape'][:2]  # Original image shape

        # Compute aspect ratio
        aspect_ratio = height / width

        # Determine base shape based on aspect ratio
        ...
        # Adjust to be divisible by size_divisor
        new_width = int(np.ceil(new_width / self.size_divisor + self.extra_pad_ratio) * self.size_divisor)
        new_height = int(np.ceil(new_height / self.size_divisor + self.extra_pad_ratio) * self.size_divisor)

        # Save target shape into results for downstream use
        results['batch_shape'] = (new_height, new_width)
