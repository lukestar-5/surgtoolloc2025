...
class RTMDetSepBNHeadModule_bce(BaseModule):
...
@MODELS.register_module()
class RTMDetHead_bce(YOLOv5Head):
    """RTMDet head.
    ...
    def loss_by_feat(
        ...
        # pred_sigmoid_max = torch.max(pred_sigmoid, dim=-1)[0]

        target_label = labels.float()
        if len(pos_inds) > 0:
            target_label[pos_inds] = assign_metrics[pos_inds]
        target_label[neg_inds] = 0.0

        # import pdb
        # pdb.set_trace()

        loss_cls = self.loss_cls(
            cls_preds.squeeze(1), target_label,
            label_weights,
        ...
        return dict(loss_bbox=loss_bbox, loss_cls=loss_cls)
