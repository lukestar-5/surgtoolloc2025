...
class RTMDetSepBNHeadModule2(BaseModule):
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> tuple:
        ...
        cls_scores = []
        bbox_preds = []
        feats_4 = self.deconv(feats[0])
        feats_new = tuple([feats_4]) + feats

        for idx, x in enumerate(feats_new):
            cls_feat = x
            reg_feat = x
